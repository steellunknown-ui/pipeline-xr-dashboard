...
async function githubFetch(token: string, url: string, options: RequestInit = {}): Promise<Response> {
  ...
  if ((res.status === 401 || res.status === 403) && PAT() && token !== PAT()) {
    return fetch(url, { ...options, headers: { ...headers, Authorization: `Bearer ${PAT()}` } });
  }
  return res;
}

export async function getUserRepos(token: string) {
  const res = await githubFetch(token, "https://api.github.com/user/repos?sort=updated&per_page=100");
...
export async function getRepoTree(token: string, owner: string, repo: string, ref: string = "HEAD") {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/trees/${ref}?recursive=1`);
...
export async function getFileContent(token: string, owner: string, repo: string, path: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/contents/${path}`);
  if (!res.ok) {
...
export async function getLatestCommitSha(token: string, owner: string, repo: string, branch: string = "main") {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/ref/heads/${branch}`);
...
export async function getCommitDetails(token: string, owner: string, repo: string, commitSha: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/commits/${commitSha}`);
...
export async function createBlob(token: string, owner: string, repo: string, content: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/blobs`, {
    method: "POST",
...
export async function createTree(token: string, owner: string, repo: string, baseTreeSha: string, path: string, blobSha: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/trees`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function createCommit(token: string, owner: string, repo: string, message: string, treeSha: string, parentSha: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/commits`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function updateBranchRef(token: string, owner: string, repo: string, branch: string, commitSha: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${branch}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function createBranch(token: string, owner: string, repo: string, branchName: string, commitSha: string) {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/git/refs`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function createPullRequest(token: string, owner: string, repo: string, title: string, body: string, head: string, base: string = "main") {
  const res = await githubFetch(token, `https://api.github.com/repos/${owner}/${repo}/pulls`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
